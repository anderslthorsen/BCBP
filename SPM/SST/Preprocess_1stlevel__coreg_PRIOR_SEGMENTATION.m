...
subjects{1}=[61 62 63 64 65 66 67 68 69 70       ...
            72 74 78 80];
...
for S = 4
    for x = subjects{1,S}
        ...
        name = strcat('SST_',num_prefix,num2str(x),timepoint);
        ext = '.nii';
        ...
        logfile{x,2} = fn;
        logfile{x,3} = char(niftis2stat(1,1));
...
matlabbatch{3}.spm.spatial.coreg.estimate.ref = ;
...
% Normalises EPI to MNI based on warps from segmented T1-weighted image
matlabbatch{4}.spm.spatial.normalise.write.subj.def = {T1_MNI_warps};
matlabbatch{4}.spm.spatial.normalise.write.subj.resample = niftistonormalize;
...
matlabbatch{6}.spm.stats.fmri_spec.sess.scans = niftis2stat;
...
matlabbatch{6}.spm.stats.fmri_spec.sess.multi_reg(1) = cfg_dep('Realign & Unwarp: Realignment Param File (Sess 1)', substruct('.','val', '{}',{2}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','sess', '()',{1}, '.','rpfile'));
...
%% Saves log file to EPI directory
    ...
    QC_filname = strcat('Logfile',timepoint,'-',time,'.xlsx');
    xlswrite(QC_filname,logfile);
    
    
    
%% Code for segmentation of the T1 image, which has already been done for all participants
