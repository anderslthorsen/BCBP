...
for S = 4
    for x = subjects{1,S}
        ...
        % Log for quality checking/debugging
        logfile{x,1} = strcat(num2str(x), timepoint);
        logfile{x,2} = fn;
        logfile{x,3} = subject_dir;
        logfile{x,4} = T1;
        logfile{x,5} = onset_file;
        logfile{x,6} = nslices;
        logfile{x,7} = ref_slice;
...
matlabbatch{5}.spm.spatial.normalise.write.subj.resample(1) = cfg_dep('Realign & Unwarp: Unwarped Images (Sess 1)', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','sess', '()',{1}, '.','uwrfiles'));
...
matlabbatch{7}.spm.stats.fmri_spec.sess.multi_reg(1) = cfg_dep('Realign & Unwarp: Realignment Param File (Sess 1)', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','sess', '()',{1}, '.','rpfile'));
