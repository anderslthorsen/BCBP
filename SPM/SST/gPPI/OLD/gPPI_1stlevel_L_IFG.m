...
if exist(['L_IFG_mask.nii'], 'file') % if there is already a VOI image for this person, move on
else                                     % if not...
    create_sphere_image(strcat(first_level_dir,'SPM.mat'),[-36,20,-1],{'L_IFG'},10) % create sphere around seed and save it in this subjects gPPI dir
    
    % Use ImCalc to mask the subject VOI by the 1st level mask
    matlabbatch{1}.spm.util.imcalc.input = {
                                            strcat(first_level_dir,'mask.nii')
                                            strcat(gPPI_dir, 'L_IFG_mask.nii')
                                            };
    matlabbatch{1}.spm.util.imcalc.output = 'L_IFG_mask';
...
P.VOI           = fullfile(gPPI_dir, ['L_IFG_mask.nii']); % path to the ROI image, created above
P.Region        = 'L_IFG'; % string, basename of output folder
...
% THis might be inhibition-related connectivity - I.E. VARIABLE OF INTEREST
