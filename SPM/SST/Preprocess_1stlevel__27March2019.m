...
for S = 1
    for x = subjects{1,S}
        ...
        if x < 10
            ...
            T1 = strcat(T1_dir,'T1_0000',num2str(x),timepoint,'\T1_0000',num2str(x),timepoint,'.nii,1');
        ...
        elseif x < 100
            ...
            T1 = strcat(T1_dir,'T1_000',num2str(x),timepoint,'\T1_000',num2str(x),timepoint,'.nii,1');
...
%%
matlabbatch{1}.spm.spatial.realignunwarp.data.scans = niftis;
...
matlabbatch{7}.spm.stats.fmri_spec.sess.multi_reg(1) = cfg_dep('Realign & Unwarp: Realignment Param File (Sess 1)', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','sess', '()',{1}, '.','rpfile'));
