#!/bin/bash
set -euo pipefail
# Command above ensures script stops if an error occurs or an undefined variable is called
...
# Note: for multishell DTI? delete?
# niter = number of iterations. DTI-TK recommends 
#Niter=5
#bshell=2000
...
#home=/s4ever/anwG/NP/yvdwerf/data_OBS/analysis/DWI_methods/b${bshell}
...
if [ ! -f DWI_${subj}_FA.nii.gz ]; then
...
for subj in `cat subjects.txt`; do mv ${subj} ${home}; done
